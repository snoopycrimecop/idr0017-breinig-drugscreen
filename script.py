...
# jamoore@necromancer /ome/data_repo/from_IDR/screen/pgpc/
#     2011-11-17_X-Man_LOPAC_X01_LP_S01_1 $ cat test.screen
PLATE = """[Plate]
Name = %s
Rows = 16
Columns = 20
            ...
            for col in range(1, 25):
                ...
                for fld in range(1, 5):
                    for idx, ch in enumerate(("DAPI", "Cy3")):
                        source = x + "/%s - %s(fld %s wv %s - %s).tif" % (
                            row, col, fld, ch, ch)
                        source = abspath(source)
                        if not exists(source):
                            raise Exception(source)
                        target = d + "/%s - %s(fld %s wv c%s).tif" % (
                            row, col, fld, idx)
                        if islink(target):
                            remove(target)
